...
var state = JSON.parse(
  fs.readFileSync(__dirname + '/state.json', { encoding: 'utf8' })
);
...
function activatePlugin(plugin) {
  ...
  for (var evt in plugins[plugin].customEvents) {
    client.on(evt, function(channel) {
      if (channel[0] === '#' && state[channel].plugins.indexOf(plugin) === -1)
        return;

      plugins[plugin].customEvents[evt].apply(
        plugins[plugin].customEvents,
        arguments
      );
    });
  }
  ...
  if (plugins[plugin].join) {
    for (var chan in client.chans) {
      // if we're already connected but the plugin got reloaded, refresh it
  ...
  if (nick === client.nick) {
    ...
    for (var i = 0; i < channels.length; ++i) {
      if (
        ...
        Object.keys(client.chans[channels[i]].users).length === 1
      ) {
...
client.on('part', function(channel, nick) {
  if (nick === client.nick) {
    ...
    Object.keys(client.chans[channel].users).length === 2
  ) {
...
client.on('invite', function(channel, inviteUser) {
  client.whois(inviteUser, function(info) {
    if (!info || !info.channels || info.channels.indexOf('@' + channel) === -1)
      return;
...
function sendCommandsNotice(activatedPlugins, from) {
  ...
  client.notice(
    from,
    'I support the following commands: ' + commandNames.join(', ')
  );
  client.notice(
    from,
    'Use !man with a specific command to get more information about it.'
  );
}
...
function sendManualEntryNotice(activatedPlugins, from, command) {
  if (command.length === 0) {
    client.notice(
      from,
      '!man: missing command operand. Try !man man for more help.'
    );
    return;
  }
  ...
  if (command === 'man') {
    client.notice(
      from,
      'Returns a notice with more information about the given command. Usage: !man COMMAND'
    );
    return;
  }
  ...
  fs.watch(
    ...
    function(evt) {
      ...
      delete require.cache[
        module._resolveFilename(__dirname + '/plugins/' + plugin)
      ];
...
(function() {
  ...
  fs.watch(__dirname + '/config.json', function() {
    ...
    console.log('changed');
    watchTimeout = setTimeout(function() {
      fs.readFile(__dirname + '/config.json', { encoding: 'utf8' }, function(
        err,
        data
      ) {
        console.log('read');
