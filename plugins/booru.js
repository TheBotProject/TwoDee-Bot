...
module.exports = function(client) {
          ...
          requestAndParse(
            ...
            function(err, res) {
              ...
              if (res.posts.post[0].$.file_url.startsWith('//')) {
                res.posts.post[0].$.file_url =
                  'https:' + res.posts.post[0].$.file_url;
              }
              if (broadcast) {
                request.head(
                  {
                    url: res.posts.post[0].$.file_url,
                    headers: { Referer: res.posts.post[0].$.file_url },
                  },
                  function(err, resp) {
                    if (
                      !err &&
                      ...
                      client.say(
                        channel,
                        (res.posts.post[0].$.rating &&
                        res.posts.post[0].$.rating !== 's'
                          ? '[\x0304NSFW\x03] '
                          : '') +
                          (host +
                            '/?page=post&s=view&id=' +
                      ...
                      client.emit('commands:image', channel, {
                        image: res.posts.post[0].$.file_url,
                      });
                    } else if (times) {
                      retry(times - 1);
                    } else {
  ...
  return {
    ...
    help: {
      catgirl:
        "return a random catgirl (tagged 'cat_ears') from safebooru.org. Extra tags are optional. Usage: !catgirl [TAGS...]",
      sb:
        'return a random image from safebooru.org. Extra tags are optional. Usage: !sb [TAGS...]',
      gb:
        'return a random image from gelbooru.com. Extra tags are optional. Usage: !gb [TAGS...]',
    },
  };
};
