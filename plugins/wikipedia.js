...
function queryWikipedia(title, cb) {
  ...
  var url =
    ...
    'titles=' +
    encodeURIComponent(title); // A list of titles to work on; Separate values with '|'
  ...
  request.get(url, function(err, resp, body) {
    if (err) {
      cb(
        new Error(
          'Something went wrong while querying Wikipedia: ' + err.message
        ),
        null
      );
    ...
    if (resp.statusCode < 200 || resp.statusCode >= 300) {
      cb(
        new Error(
          'Something went wrong while querying Wikipedia: ' +
            resp.statusCode +
            ': ' +
...
var wikipedia =
  'Returns the wikipedia.org article for the given terms. Usage: !%s SEARCH';
var help = {};
        ...
        queryGoogle(msg, function(err, title) {
          ...
          queryWikipedia(title, function(err, result) {
            ...
            try {
              ...
              client.say(
                to,
                'Something went terribly wrong. The authorities have been contacted.'
              );
            }
          });
        });
