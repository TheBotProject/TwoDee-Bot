...
try {
  var clientId = fs
    .readFileSync(__dirname + '/.soundcloud', { encoding: 'utf8' })
    .trim();
} catch (err) {
...
function parseLinks(str) {
  ...
  for (var i = 0; i < strArr.length; i++) {
    ...
    if ((match = strArr[i].match(songRegex))) {
      ...
      if (song.title.match(badTitleRegex)) continue;
  ...
  google(url, function(err, next, links) {
    if (err) {
      if (err.status) {
        cb(
          new Error(
            'Something went wrong while searching on Google: ' +
              err.status +
              ': ' +
        ...
        cb(
          new Error(
            'Something went wrong while searching on Google: ' + err.message
          ),
          null
        );
      }
...
function querySoundcloud(url, cb) {
  request.get(url, function(err, resp, body) {
    if (err) {
      cb(
        new Error(
          'Something went wrong while querying Soundcloud: ' + err.message
        ),
        null
      );
    ...
    if (resp.statusCode < 200 || resp.statusCode >= 300) {
      cb(
        new Error(
          'Something went wrong while querying Soundcloud: ' +
            resp.statusCode +
            ': ' +
...
function format(data) {
  ...
  return (
    ...
    '] ' +
    title +
    (genre ? ' \x0302#' + genre + '\x03' : '') +
    (count ? ' (' + count + ' tracks)' : '') +
    ' [' +
    durationFormat(Math.ceil(duration / 1000)) +
    '] [ ' +
    link +
    ' ]'
  );
