  ...
  return {
    commands: {
      repost: function(from, to, msg) {
        ...
        request.get(
          'http://redditbooru.com/images/?sources=-1&imageUri=' +
            encodeURIComponent(msg),
          function(err, res) {
            if (err || res.statusCode >= 400) {
              client.say(
                to,
                'Error while retrieving repost information for ' + msg
              );
              return;
            }
            try {
    ...
    help: {
      repost:
        'Searches redditbooru.com for possible reposts of the given image on reddit. Usage: !repost URL',
    },
  };
};
