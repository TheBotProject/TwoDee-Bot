...
module.exports = function(client) {
  function searchAnime(query, cb) {
    ...
    google('site:myanimelist.net/anime/ ' + query, function(err, res) {
      if (err) {
        cb(
          new Error(
            'Attempt to google "' +
  ...
  return {
      ...
      mal: function(from, channel, message) {
        ...
        searchAnime(message, function(err, data) {
          if (err) {
            ...
            client.say(
              channel,
              ent.decode(data.title) +
                ' (' +
                (data.episodes ? data.episodes : '?') +
                ' episodes) - http://myanimelist.net/anime/' +
                data.id
            );
          } else {
            client.say(
              channel,
              "Couldn't parse anime info, here's the link though: http://myanimelist.net/anime/" +
                data
            );
          }
        });
    ...
    help: {
      mal:
        'Returns the myanimelist.net page for the requested show. Usage: !mal SHOW',
    },
  };
};
