...
module.exports = function(client) {
  ...
  function getBest(entries) {
    var seedersWeight = 1;
    var leechersWeight = 0.5;
    var ageWeight = -1 / 1000 / 60 / 60;
    // score is # of seeders + half of # leechers - age in hours
    var bestScore =
      entries[0].seeders * seedersWeight +
      entries[0].leechers * leechersWeight +
      entries[0].age * ageWeight;
    var bestEntry = entries[0];

    for (var i = 1; i < entries.length; i++) {
      var score =
        entries[i].seeders * seedersWeight + entries[i].age * ageWeight;
      if (score > bestScore) {
        bestScore = score;
        bestEntry = entries[i];
      }
  ...
  return {
    commands: {
        ...
        requestAndParse(
          ...
          function(err, data) {
            ...
            if (!entries || entries.length === 0) {
              client.say(
                channel,
                "No results for '" + msg + "'. Try !nyaan or !nyaall?"
              );
              return;
            }
      ...
      nyaall: function(from, channel, msg) {
        ...
        requestAndParse(
          defaultBaseURL + '?page=rss&term=' + encodeURIComponent(msg),
          function(err, data) {
            if (err) {
              client.say(channel, 'No response. Please try again.');
              return console.error('Error getting data: ' + err);
            ...
            client.notice(
              from,
              defaultBaseURL + '?page=search&term=' + encodeURIComponent(msg)
            );
          }
        );
      },
      ...
      nyaan: function(from, channel, msg) {
        ...
        requestAndParse(
          sukebeiBaseURL + '?page=rss&term=' + encodeURIComponent(msg),
          function(err, data) {
            if (err) {
              client.say(channel, 'No response. Please try again.');
              return console.error('Error getting data: ' + err);
            ...
            if (!entries || entries.length === 0) {
              client.say(
                channel,
                "No results for '" + msg + "'. Try !nyaa or !nyaall?"
              );
              return;
            }
            ...
            client.notice(
              from,
              sukebeiBaseURL + '?page=search&term=' + encodeURIComponent(msg)
            );
          }
        );
      },
    ...
    messageHandler: function(from, channel, message) {
      ...
      while ((match = re.exec(message))) {
        getData(
          match[1] === 'sukebei' ? sukebeiBaseURL : defaultBaseURL,
          match[4],
          function(nsfw, data) {
            var dataString =
              (nsfw ? '\x0304[NSFW]\x03 ' : '') +
              formatData({
                ...
                guid:
                  (nsfw ? sukebeiBaseURL : defaultBaseURL) +
                  '?page=view&tid=' +
                  data.id,
                link:
                  (nsfw ? sukebeiBaseURL : defaultBaseURL) +
    ...
    help: {
      nyaa:
        'Returns a torrent link for the given name from nyaa.se for english-translated anime. Usage: !nyaa ANIME',
      nyaall:
        'Returns a torrent link for the given name from nyaa.se. Usage !nyaall NAME',
      nyaan:
        'Returns a torrent link for the given name from sukebei.nyaa.se. Usage !nyaan NAME',
    },
  };
};
