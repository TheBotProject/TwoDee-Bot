...
try {
  ...
  console.error(
    "Couldn't read saucenao api key: " +
      e +
      '. Proceeding with anonymous account.'
  );
  api_key = '';
}

var sauceTxt =
  'Looks on pixiv.net for the source of the given image. Usage: !%s URL';
var help = {};
      ...
      if (err) {
        ...
        request(
          ...
          function(err, r, data) {
            ...
            if (
              data.match('Search Rate Too High.') ||
              data.match('Daily Search Limit Exceeded.')
            ) {
            ...
            for (var i = 0; i < results.length && i < 3; ++i) {
              ...
              var similarity = parseInt(
                $('.resultsimilarityinfo', result).text(),
                10
              );
              ...
              var link = $('.resultcontentcolumn a', result)
                .eq(0)
                .attr('href');
