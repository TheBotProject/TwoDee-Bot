...
module.exports = function(client) {
  function uploadAlbum(albumTitle, imageIds) {
    ...
    return postRequest(uploadRequest).then(function(response) {
      var jsonResponse = JSON.parse(response.body);

      if (jsonResponse && (jsonResponse.redirect || jsonResponse.route)) {
        return (
          jsonResponse.redirect || 'http://redditbooru.com' + jsonResponse.route
        );
      }
  ...
  function uploadImage(imageUrl) {
    ...
    var uri =
      'http://redditbooru.com/upload/?action=upload&force=true&imageUrl=' +
      encodeURIComponent(imageUrl);

    return postRequest(uri).then(function(response) {
      var jsonResponse = JSON.parse(response.body);
  ...
  function retrievePixivImages(images) {
    ...
    images.forEach(function(item) {
      ...
      if (match) {
        pixivPromises.push(
          pixiv.getPixivImage(match[4]).then(pixiv.getImageUrlsFromManga)
        );
      }
      ...
      if (match) {
        pixivPromises.push(
          pixiv.getPixivImage(match[2]).then(pixiv.getImageUrlsFromManga)
        );
      }
    });
  ...
  return {
      ...
      rb: function(from, channel, message) {
        ...
        if (length > 1 && !(albumTitle = checkTitle(message, length))) {
          client.say(
            channel,
            from +
              ", your album doesn't have a title! (Make sure you put quotes around your title.)"
          );
          return;
        }
        ...
        retrievePixivImages(splitUploads[0])
          .then(
            function(results) {
              ...
              var hasCorrectTitle =
                albumTitle ||
                (albumTitle = checkTitle(message, uploads.length));
            ...
            function(error) {
              ...
              if (error === pixiv.errors.missingImageCount) {
                error =
                  'something went wrong while trying to get your images from Pixiv';
              }
    ...
    help: {
      rb:
        'Accepts images and pixiv links and uploads them to redditbooru.com. Using multiple images requires a title between quotes. Usage: !rb [TITLE] IMAGE...',
    },
  };
};
