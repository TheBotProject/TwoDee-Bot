...
module.exports = function(client) {
  ...
  return {
    commands: {
      choice: function(from, channel, message) {
        ...
        if ((match = message.match(/"/g)) && match.length % 2 === 1) {
          client.say(
            channel,
            from + ', you seem to be missing a " in your choices.'
          );
          return;
        }
        ...
        if (choices.length === 0) {
          client.say(
            ...
            'Sorry ' +
              from +
              ', something went wrong while trying to parse your choices.'
          );
          return;
        }
    ...
    help: {
      choice:
        'Returns a randomly selected element from the provided choices. Splitting happens between spaces, or single and double quotes. Usage: !choice OPTIONS...',
    },
  };
};
