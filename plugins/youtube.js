...
var ytText =
  'Searches youtube for the specified terms and returns a video. Usage: !%s SEARCH';
var help = {};
  ...
  youtube.videos.list(
    ...
    function(err, videoListResponse) {
      if (err) {
        cb(
          new Error(
            'Error while trying to get data for https://youtu.be/' +
              id +
              ': ' +
              err.message
          ),
        ...
        console.error(
          'Bad response from Youtube: ' + JSON.stringify(videoListResponse)
        );
        cb(
          new Error(
            'Got a bad response from Youtube, while trying get data for https://youtu.be/"' +
              id +
              '".'
          ),
          null
  ...
  youtube.search.list(
    ...
    function(err, searchListResponse) {
      if (err) {
        cb(
          new Error(
            'Error while trying to look up "' + term + '": ' + err.message
          ),
          null
        );
        ...
        // we should never reach this point
        console.error(
          'Bad response from Youtube: ' + JSON.stringify(searchListResponse)
        );
        cb(
          new Error(
            'Got a bad response from Youtube, while trying search for "' +
              term +
              '".'
          ),
          null
