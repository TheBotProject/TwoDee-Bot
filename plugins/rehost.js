    ...
    return {
      commands: {
        imgur: function(from, to, message) {
          ...
          request.head(
            { url: message, headers: { Referer: message } },
            function(err, resp) {
              if (!err && resp.statusCode >= 200 && resp.statusCode < 300) {
                if (
                  resp.headers['content-type'] === 'image/gif' &&
                  resp.headers['content-length'] > 2048000
                ) {
                  client.say(
                    to,
                    'Size limit for gifs is 2 MB, ' +
                      from +
                      '. Try http://awwni.me.'
                  );
                  return;
              ...
              upload(message, function(e, r, b) {
                if (e) {
                  ...
                  client.say(
                    ...
                    'Sorry, ' +
                      from +
                      ', something went wrong. Try http://awwni.me.'
                  );
                } else if (
                  resp.statusCode >= 200 &&
                  resp.statusCode < 300 &&
                  ...
                  client.say(
                    to,
                    "I don't see anything wrong, but imgur says: " +
                      b.data.error
                  );
                } else {
                  client.say(
                    to,
                    "Something I don't know what went wrong. Try http://awwni.me."
                  );
                }
              });
            }
